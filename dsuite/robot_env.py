...
from dsuite.simulation.sim_scene import SimScene, SimBackend
from dsuite.simulation.renderer import RenderMode
...
class RobotEnv(gym.Env, metaclass=abc.ABCMeta):
    ...
    def _initialize_state_space(self) -> gym.Space:
        ...
        The default implementation uses a Tuple of the simulation's qpos and
        qvel dimensions.
        """
        return spaces.Tuple((
            make_box_space(-np.inf, np.inf, shape=(self.model.nq,)),
            make_box_space(-np.inf, np.inf, shape=(self.model.nv,)),
        ))
