...
```
$> python dsuite.scripts.play -e DClawTurnFixed-v0
```
"""
...
class PlayShell(cmd.Cmd):
    ...
    def _load_env(self,
        ...
        if env_params:
            dsuite.set_env_params(env_name, env_params)
            ...
            with self._env_lock:
                ...
                if self._load_env_name is not None:
                    if self._env is not None:
                        self._env.close()
                    dsuite.set_env_params(self._load_env_name,
                                              self._load_env_params)
                    self._env = gym.make(self._load_env_name)
                    self._env.reset()
